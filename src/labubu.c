...
// Function to safely print labubu art with error handling
int safe_print_labubu(const char* labubu_art) {
    // Validate input parameter
    if (labubu_art == NULL) {
        fprintf(stderr, "Error: NULL pointer passed to safe_print_labubu\n");
        return -1;
    }
    
    // Check for valid string length and prevent buffer overflow
    size_t art_length = strlen(labubu_art);
    if (art_length > 5000) {  // Reasonable upper limit
        fprintf(stderr, "Error: String too long for safe printing\n");
        return -1;
    }
